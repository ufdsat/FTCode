...
models = [
              Pipeline([('normalizer', StandardScaler()),  # normalize data
                      ('clf', LogisticRegression(random_state=1, max_iter=100000))  # fit Logistic regression model
              ]),    # pipeline for Logistic Regression
              Pipeline([('normalizer', StandardScaler()),  # normalize data
                  ('clf', SVC(random_state=1))  # fit SVM
              ]),    # pipeline for SVM
...
for file in files:
    ...
    # Outer CV train-validation on 9 participant, test on the other 1
...
plt.ylabel('Accuracy')
plt.title('Accuracy vs. Window-Length [IAR]')
plt.ylim([0.35, 0.72])
